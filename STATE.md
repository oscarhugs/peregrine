...
- **Next session (start fresh; don't continue the long one):** 1) run the storyboarder (Sonnet) on script.md → storyboard.json; 2) Claude writes the video-assembler Codex brief (queue item 4) from the storyboard schema; 3) editor publish pass → publish.md; 4) thumbnail hero images (T1 arrow: use up or none, not down). Oscar: the record voice reference once Codex's voice tool lands.
